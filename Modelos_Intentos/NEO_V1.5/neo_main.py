...
def procesar_comando_completo(comando):
    ...
    if plan:
        # Explicar qué hará (con voz)
        explicacion = plan.get('explicacion', '')
        if explicacion:
            print(f"💬 NEO: {explicacion}")
            neo_voz.neo_hablar(explicacion)
        ...
        # Confirmar con voz
        if exito:
            neo_voz.neo_hablar("Listo")
            # Guardar en memoria
            explicacion = plan.get('explicacion', 'Comando ejecutado') if plan else 'No entendí'
            neo_memoria.guardar_interaccion(comando, explicacion)        
        return exito
    else:
